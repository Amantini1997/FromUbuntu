...
def batch_gradient_descent():
    ...
    print metrics.mean_absolute_error(y, predict_arr(X)), W

def stochastic_gradient_descent():
    for i in range(M):
        update_weights_stochastic(y[i], predict(X[i]), X[i])
    print metrics.mean_absolute_error(y, predict_arr(X)), W
...
plt.plot(X, predict_arr(X), "b-", label="batch")
W = [0.0, 0.0]
stochastic_gradient_descent()
plt.plot(X, predict_arr(X), "k-", label="stoch")
plt.legend()
plt.show()
plt.close()