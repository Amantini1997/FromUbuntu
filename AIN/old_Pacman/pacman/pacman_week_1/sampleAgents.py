...
class HungryAgent(Agent):
    ...
    def getAction(self, state):
        ...
        if (self.lastMinimumDistance and
           sum(xyDistanceToClosestFood) >= sum(self.lastMinimumDistance) and
           pacman != self.lastBestFoodPosition):

            print "LMD: ", self.lastMinimumDistance
            print "LBF: ", self.lastBestFoodPosition
            print "ActualDTCF: ", xyDistanceToClosestFood
...
def getBestNextMoves(legal, closestPointDistancesXY):
    ...
    print "\t\t+++++++++    CLOSEST POINT TO DISTANCE", closestPointDistancesXY
